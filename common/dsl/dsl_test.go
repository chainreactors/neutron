...
func TestDslFunctionSignatures(t *testing.T) {
	createSignatureError := func(signature string) string {
		return fmt.Errorf("%v. correct method signature %q", ErrInvalidDslFunction, signature).Error()
	}
...
func TestDslExpressions(t *testing.T) {
	dslExpressions := map[string]interface{}{
		...
		`xray_dedupe_path("http://example.com/vpn", "vpn/index.html")`: "index.html",
		`xray_dedupe_path("http://example.com/geoserver", "web/")`:     "web/",
		`xray_dedupe_path("http://example.com/bi/", "/bi/?proc=1")`:    "?proc=1",
		`xray_dedupe_path("http://example.com/", "/console/login")`:    "console/login",
