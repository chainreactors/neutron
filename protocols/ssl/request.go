...
func (r *Request) responseToDSLMap(data map[string]interface{}, target string, conn *tls.Conn, state *tls.ConnectionState) {
	...
	data["port"] = port
	data["matched"] = target
	data["probe_status"] = true
	data["tls_connection"] = "ctls"
	...
	// response: JSON summary for `part: response` and DSL over the whole structure
...
// tlsVersionValue maps a textual version to the crypto/tls constant.
